  ...
  <div>
    ...
    <div v-if="addBar !== null">
      <v-form class="form_class" ref="form" lazy-validation>
        <v-text-field label="Bar name" v-model="bar.name" required></v-text-field>
        <v-text-field label="Address" v-model="bar.address" required></v-text-field>
    ...
    <v-btn class="return" id="return" @click="Reset"></v-btn>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    addNewBarToDB() {
      axios
        .post("http://217.101.44.31:8084/api/public/bar/addBar", {
          name: this.bar.name,
...
#return {
  ...
  background-image: url("../../../assets/images/return_icon.png");
}
